...
pub enum TaskType{
    InviteOnly {invited_accounts: HashSet<AccountId>,valid_till: Timestamp},      // keeps track of invited accounts if an invite only project and validity date till if which if no-one accepts then company can claim refund
...
pub struct Task{
    ...
    pub required_skills: String,                // required skills for the task in a space seperated format
